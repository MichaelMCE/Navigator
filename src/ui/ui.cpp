...
#define UI_WIDGETOBJS_TOTAL		17
static ui_widget_t *widgetObjs[UI_WIDGETOBJS_TOTAL] = {WIDGET(&button_config), 0, 0, 0, 0, 0, WIDGET(&button_overlayDetail), 0, WIDGET(&button_logCtrl), 0/*logctrl*/, WIDGET(&button_zoom[0]), WIDGET(&button_zoom[1]), 0/*logs*/, WIDGET(&button_updown[0]), WIDGET(&button_updown[1]), WIDGET(&button_logRefresh), WIDGET(&button_logLoad)};
...
int uiButtons_cb (ui_widget_t *widget, const uint8_t id, const uint32_t flags, const uint32_t msg, const int32_t var1, const int32_t var2)
{
	...
	case UI_ID_BUTTON_GNSS:
		ui_disable(NULL, UI_ID_PANEL_RECEIVER);
		ui_enable(NULL, UI_ID_PANEL_GNSS);
		ui_disable(NULL, UI_ID_BUTTON_CONFIG);
		return 1;
	...
	case UI_ID_BUTTON_LOGCTRL:
		...
		ui_disable(NULL, UI_ID_PANEL_GNSS);
		ui_disable(NULL, UI_ID_PANEL_LOGS);
...
FLASHMEM static ui_button_t *ui_panel_addButton (ui_panel_t *panel, const uint8_t id, const uint8_t flags, ui_widget_cb_t callback, const char *text, const uint8_t position)
{
	...
	for (int i = 0; i < panel->widget.children.total; i++){
		...
		if (!button->widget.id){
			...
			button->rect.y = 15 + (60 * (position-1));
...
FLASHMEM static void ui_panelBuild_receiverRestart ()
{
...
{
	ui_panel_t *receiver = ui_panel_create(UI_ID_PANEL_RECEIVER, 6, uiPanel_cb, 20, (VHEIGHT-(6*60))-20, 230, 6*60);
	if (!receiver) return;
	...
	ui_panel_addButton(receiver, UI_ID_BUTTON_GNSS,        UI_WIDGET_FLAG_HASPANEL, uiButtons_cb, "GNSS",      6);
...
FLASHMEM static void ui_panelBuild_gnss ()
{
	ui_panel_t *panel = ui_panel_create(UI_ID_PANEL_GNSS, 7, uiPanel_cb, 20, (VHEIGHT-(7*60))-20, 230, 7*60);
	if (!panel) return;
	...
	ui_enable(0, UI_ID_BUTTON_GPS);
	ui_enable(0, UI_ID_BUTTON_GALILEO);
	ui_enable(0, UI_ID_BUTTON_BEIDOU);
	ui_enable(0, UI_ID_BUTTON_GLONASS);
	ui_enable(0, UI_ID_BUTTON_SBAS);
	ui_enable(0, UI_ID_BUTTON_QZSS);
	ui_enable(0, UI_ID_BUTTON_IMES);
}
...
{
	ui_panel_t *panel = ui_panel_create(UI_ID_PANEL_DISPLAY, 2, uiPanel_cb, 20, (VHEIGHT-(2*60))-20, 160, 2*60);
	if (!panel) return;
	
	widgetObjs[4] = WIDGET(panel);
	
	ui_panel_addButton(panel, UI_ID_BUTTON_MODE, 0, uiButtons_cb, "Mode", 1);
	ui_panel_addButton(panel, UI_ID_BUTTON_OFF,  0, uiButtons_cb, "Off",  2);
...
{
	ui_panel_t *panel = ui_panel_create(UI_ID_PANEL_LOGS, 1, uiLogs_cb, 100, 14, 760, 440);
	if (!panel) return;
...
FLASHMEM static void ui_panelBuild_ctrl ()
{
	ui_panel_t *panel = ui_panel_create(UI_ID_PANEL_MAPCTRL, 4, uiPanel_cb, 20, (VHEIGHT-(4*60))-20, 230, 4*60);
	if (!panel) return;
	...
	ui_panel_addButton(panel, UI_ID_BUTTON_RECEIVER, UI_WIDGET_FLAG_HASPANEL, uiButtons_cb, "Receiver", 3);
	ui_panel_addButton(panel, UI_ID_BUTTON_MPU,      UI_WIDGET_FLAG_HASPANEL, uiButtons_cb, "MPU",      4);
...
int ui_input (const int32_t x, const int32_t y, const uint32_t flags)
{
	...
	if (!ret){
		...
		}else if (ui_isEnabled(NULL, UI_ID_PANEL_GNSS)){
			ui_disable(NULL, UI_ID_PANEL_GNSS);
			return 1;
		}
	}
